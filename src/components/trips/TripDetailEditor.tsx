...
export function TripDetailEditor({ trip, onSave, onCancel, onSaved, onSaving }: {
  ...
  // A callsheet can describe one filming location without a complete journey.
  // Keep that valid after the reviewed document has been saved as a trip, too.
  const isDocumentTrip = Boolean(trip.callsheet_job_id);
  const minStops = isDocumentTrip ? 1 : 2;
  const [date, setDate] = useState(trip.date);
  const [project, setProject] = useState(trip.project);
  const [purpose, setPurpose] = useState(trip.purpose);
  const [route, setRoute] = useState(trip.route.length ? [...trip.route] : Array<string>(minStops).fill(''));
  ...
  return <form className="flex h-full min-h-0 flex-col" onSubmit={async event => {
    ...
    if (saving) return;
    const km = isDocumentTrip && !distance.trim() ? 0 : parseLocaleNumber(distance);
    ...
    <fieldset disabled={saving} className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
      ...
      <div className="space-y-2"><Label>{t('tripDetail.route')}</Label>{route.map((stop, index) => <div className="flex items-start gap-2" key={index}>
        ...
        <Button type="button" variant="ghost" disabled={route.length <= minStops} aria-label={`${t('trips.delete')} ${index + 1}`} onClick={() => setRoute(prev => prev.filter((_, i) => i !== index))}>×</Button>
      ...
      <div><Label htmlFor="detail-edit-distance">{t('tripModal.distance')}</Label><Input id="detail-edit-distance" inputMode="decimal" required={!isDocumentTrip} value={distance} onChange={e => setDistance(e.target.value)} /></div>
      <div><Label htmlFor="detail-edit-passengers">{t('trips.passengers')}</Label><Input id="detail-edit-passengers" inputMode="numeric" required={!isDocumentTrip} value={passengers} onChange={e => setPassengers(e.target.value)} /></div>
      </div>
