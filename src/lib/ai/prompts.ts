...
export function buildUniversalExtractorPrompt(text: string) {
  return [
    ...
    "",
    "── date ──",
    "Main shooting day date printed on the FIRST PAGE. Normalize to YYYY-MM-DD. Search ALL pages for locations belonging to this date; do not impose a page cutoff.",
    LOCATION_DAY_RULE,
    ...
    "  • Also check the DOCUMENT TEXT EXCERPT section below for projectName clues.",
    ...
    "  • The company often appears ONLY as a LOGO image (header/footer, no plain text) —",
    "    read the company name from the logo in the attached document.",
    ...
    "dayEvidence: copy the address block and the heading/column text that establishes its day, verbatim, on separate lines when they are separate blocks. They need not be adjacent in the source. Do not paraphrase or add connecting words. Include its associated Maps link if present. Never copy a different day's heading as evidence.",
    ...
    "Never return transit directions, floor numbers, phone numbers or standalone notes as addresses.",
    "",
    "COLLECT these logistics blocks with their own labels; do NOT treat them as filming destinations:",
    ...
    "  Hospital, Arzt, Medic.",
    "Keep these addresses in locations for classification and separate storage. Never omit a block merely because it is logistics.",
    "If logistics and filming share an address, preserve their separately labeled blocks so the app can distinguish their roles.",
    "",
    "VERBATIM RULE:",
    ...
    "  (e.g. 'Operring 2' → 'Opernring 2') and append the city/postal code",
    "  if printed elsewhere in the document.",
    "• NEVER a different place, NEVER invented house numbers.",
    "• Empty string if the printed address needs no correction.",
    "",
    "BUILDINGS: Return building name + address. NOT sub-rooms inside (Foyer, Bar, Keller).",
    "SUB-AREAS: scene spots inside a venue/park WITHOUT their own street address (Wiese, Tennisplatz, Yogaplatz, Spielplatz…) are NOT locations — return only the venue/address they belong to, once.",
    "",
    "DOCUMENT SECTION EXCLUSIONS (CRITICAL):",
    "Analyze the full document, but completely ignore content inside clearly labeled sections for:",
    "  • Another/future shooting day: 'NEXT DAY', 'NEXT SHOOTING DAY', 'TOMORROW',",
    "    'NÄCHSTER DREHTAG', 'FOLGETAG', 'MORGEN', 'PRÓXIMO DÍA', 'SIGUIENTE DÍA'.",
    "  • Appendices/attachments: 'APPENDIX', 'ANNEX', 'ATTACHMENT', 'ANHANG', 'ANLAGE',",
    "    'ANEXO', 'ANEXOS', 'ADJUNTO', 'ADJUNTOS'.",
    "  • Contact or crew directories: 'CONTACTS', 'CONTACT LIST', 'CREW LIST', 'PHONE LIST',",
    "    'KONTAKTE', 'KONTAKTLISTE', 'STABLISTE', 'TELEFONLISTE', 'CONTACTOS', 'LISTA DE CONTACTOS'.",
    ...
    "Only the main unit for the shooting date printed on the FIRST PAGE is eligible for routes. Collect labeled filming and logistics blocks belonging to that",
