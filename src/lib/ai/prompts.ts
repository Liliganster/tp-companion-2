...
export function buildUniversalExtractorPrompt(text: string) {
  return [
    ...
    'Read shooting dates in ANY language and layout, including translated/abbreviated month names, weekday prefixes, ordinals, DD.MM.YYYY, DD/MM/YYYY, MM/DD/YYYY and year-first dates. Use the document language and context to distinguish day/month order. A different format is not missing data. Convert a clear date to ISO; if genuinely ambiguous preserve dateRaw, leave date empty, and still extract ALL remaining fields and locations. Never abandon the document because of a date.',
