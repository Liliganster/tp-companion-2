...
it('stops displaying an abandoned request as processing, even on repeated polls', () => {
  for (const now of [90_000, 120_000, 300_000]) expect(resolveCallsheetProcessingState(job, false, now).status).toBe('needs_review');
});
it('keeps a live request and a recently claimed job in progress', () => {
  ...
  expect(resolveCallsheetProcessingState(job, false, 89_000)).toBe(job);
});
