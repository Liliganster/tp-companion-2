...
export function resolveCallsheetProcessingState<T extends { status: string; created_at?: string | null; processing_started_at?: string | null; processed_at?: string | null; needs_review_reason?: string | null }>(job: T, requestActive: boolean, now = Date.now()): T {
  ...
  return { ...job, status: 'failed', needs_review_reason: job.status === 'processing'
    ? 'La extracción no terminó dentro del tiempo de espera. El documento se conserva; puedes revisarlo manualmente.'
    : 'La carga o puesta en cola se interrumpió. Revisa el documento o vuelve a subirlo.' };
}
